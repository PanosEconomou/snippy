 ...
 * ║   ━━┃┃ ┃┃┏━┛┏━┛━┏┛                                                           ║ *
 * ║   ━━┛┛ ┛┛┛  ┛   ┛                                                            ║ *
 * ║   keyboard.c | use udev, evdev, and xkb to find and listen to keyboards      ║ *
 * ║                                                                              ║ *
 * ╚══════════════════════════════════════════════════════════════════════════════╝ */
...
int keyboard_open(struct keyboard* keyboard, const char* node, 
    ...
    *keyboard = (struct keyboard){ 0 };
    ...
    int error = libevdev_new_from_fd(keyboard->file_descriptor,
                                     &keyboard->device);
...
int keyboard_set_open_many(struct keyboard_set* keyboards, 
                           ...
                           size_t found, struct xkb_keymap* keymap) {
    ...
    for (size_t i = 0; i < found && n_open < MAX_KEYBOARDS; i++) {
        ...
        if (error < 0) { 
            /* NOTE: if you can't open a keyboard, don't burn the universe
             *       just tell the user you coulnd't and move on. */
...
int keyboard_set_open_all(struct keyboard_set* keyboards, struct xkb_keymap* keymap) {
    ...
    return keyboard_set_open_many(keyboards, nodes, (size_t)found, keymap);
}
...
void keyboard_set_remove(struct keyboard_set* keyboards, size_t index) {
    ...
    /* NOTE: remove the keyboard and add the last one in its place.
     *       order doesn't matter anyway. */
...
enum keyboard_status keyboard_next_event(struct keyboard* keyboard, 
    ...
    int status = libevdev_next_event(keyboard->device, 
    ...
    switch (status) {
        ...
        case LIBEVDEV_READ_STATUS_SYNC: {
            /* NOTE: there are stuff in the kernel buffer that don't make sense 
             *       discard them. */
            struct input_event discard;
            while (libevdev_next_event(keyboard->device, 
                                       LIBEVDEV_READ_FLAG_SYNC, 
                                       &discard) == LIBEVDEV_READ_STATUS_SYNC);
            return KEYBOARD_DROPPED;
        }

        default:
            /* NOTE: anything else is some unforseen error, 
             * pretend keyboa exploded */
            return KEYBOARD_GONE;
    }
}

size_t keyboard_set_pollfds(struct keyboard_set* keyboards, 
                            struct pollfd* pfds, size_t capacity){
